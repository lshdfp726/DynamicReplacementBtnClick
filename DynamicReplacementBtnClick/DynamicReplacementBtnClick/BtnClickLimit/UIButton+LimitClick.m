...
+(void)load {
    ...
    Method def    = class_getInstanceMethod([self class], defSEL);
    /*
    ...
    if (class_addMethod([self class], originSEL, method_getImplementation(def), method_getTypeEncoding(def))) {
        class_replaceMethod([self class], defSEL, method_getImplementation(origin), method_getTypeEncoding(origin));
    }else {
